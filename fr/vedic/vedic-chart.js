...
function renderD9Interpretation(d9Positions, d9LagnaSign, d1LagnaSign) {
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ${isEasy ? 'Cela revele votre vrai vous apres le mariage et dans la seconde moitie de la vie (apres 30 ans).' : 'Navamsa Lagna is in <strong>' + SIGNS[d9LagnaSign] + '</strong>. This reveals your true self after marriage and in the second half of life (after 30s).'}
            ...
            ${d9H1Planets.length > 0 ? '<br><br>' + (isEasy ? 'Il y a des energies qui influencent fortement votre personnalite apres le mariage.' : '<strong>Planets in D9 1st:</strong> ' + d9H1Planets.map(p => p.symbol + ' ' + p.name).join(', ') + ' — influencent fortement votre personnalite apres le mariage.') : ''}
        </div>
    </div>`;
    ...
    if (vargottamaPlanets.length > 0) {
        html += `<div class="interp-card">
            ...
            <div class="interp-text">
                ${isEasy ? 'Ces planetes sont exceptionnellement puissantes et agissent de maniere constante toute la vie.' : 'Planets in the same sign in both D1 and D9 are called <strong>Vargottama</strong>. Very powerful, acting consistently throughout life.'}<br><br>
                ${isEasy ? 'Energie exceptionnellement forte agissant de maniere constante toute votre vie !' : vargottamaPlanets.map(p => `<strong>${p.symbol} ${p.name}</strong>: In both D1 and D9 ${SIGNS[p.sign]} — this planet's energy is exceptionally strong!`).join('<br>')}
            </div>
        </div>`;
    }
...
function renderEducation(positions, lagnaSign) {
    ...
    if (jupiter) {
        ...
        if ([1,4,5,9].includes(jH)) text += isEasy ? '<br><br>🎓 <strong>Haute reussite academique attendue !</strong> Ecole superieure/doctorat/etudes a l\'etranger possible.' : '<br><br>🎓 <strong>Jupiter in ' + jH + 'th house — high academic achievement expected!</strong> Graduate school/PhD/study abroad possible.';
    }
...
function renderDivisionalChart(positions, lagnaSidereal, division, chartId, interpId, label, koName) {
    ...
    if (division === 10) {
        ...
        if (isEasy) {
            ...
            html += '<strong>D10 10e Maison (carriere) :</strong> ' + SIGNS[d10_10sign] + ' (ruler: ' + (RULER_NAMES[d10_10lord]||d10_10lord) + ')<br>';
        }
        ...
        if (!isEasy) {
            ...
            html += '<strong>D7 5e Maison (enfants) :</strong> ' + SIGNS[d7_5sign] + ' (ruler: ' + (RULER_NAMES[d7_5lord]||d7_5lord) + ')<br>';
        }
        ...
        const D60_DEITIES = [
            {name:'Ghora',nature:'malefic',desc:'Destruction et peur. Karma sombre des vies passees'},
            {name:'Rakshasa',nature:'malefic',desc:'Energie demoniaque. Fort desir et attachement'},
            {name:'Deva',nature:'benefic',desc:'Etre divin. Merite et benedictions des vies passees'},
            {name:'Kubera',nature:'benefic',desc:'Dieu de la richesse. Karma de construction de richesse'},
            {name:'Yaksha',nature:'benefic',desc:'Gardien de la nature. Harmonie avec la nature'},
            {name:'Kinnara',nature:'benefic',desc:'Musicien celeste. Talent artistique'},
            {name:'Bhrashta',nature:'malefic',desc:'Le dechu. Karma de chute de haut'},
            {name:'Kulaghna',nature:'malefic',desc:'Destructeur de famille. Karma lie a la famille'},
            {name:'Garala',nature:'malefic',desc:'Poison. Karma d\'actions toxiques'},
            ...
            {name:'Purishaka',nature:'malefic',desc:'Servitude. Karma de restriction des autres'},
            {name:'Apampathi',nature:'benefic',desc:'Seigneur des eaux. Purification et guerison'},
            {name:'Marut',nature:'benefic',desc:'Dieu du vent. Liberte et changement'},
            {name:'Kala',nature:'malefic',desc:'Dieu du temps. Karma du temps et de la mort'},
            {name:'Sarpa',nature:'malefic',desc:'Serpent. Servitude et attachement — incapable de lacher'},
            {name:'Amrita',nature:'benefic',desc:'Nectar d\'immortalite. Poursuite de la vie eternelle'},
            {name:'Indu',nature:'benefic',desc:'Lune. Sensibilite et intuition'},
            {name:'Mridu',nature:'benefic',desc:'Le doux. Douceur et compassion'},
            {name:'Komala',nature:'benefic',desc:'Le delicat. Art et beaute'},
            {name:'Heramba',nature:'benefic',desc:'Avatar de Ganesha. Surmonter les obstacles'},
            ...
            {name:'Bala',nature:'benefic',desc:'Force. Fortitude et courage'},
            {name:'Vishwakarma',nature:'benefic',desc:'Architecte cosmique. Construction et creation'},
            {name:'Tamasa',nature:'malefic',desc:'Obscurite. Karma d\'ignorance'},
            {name:'Kanchana',nature:'benefic',desc:'Or. Purete et valeur'},
            {name:'Varaha',nature:'benefic',desc:'Avatar sanglier de Vishnu. Salut'},
            {name:'Ramasala',nature:'benefic',desc:'Demeure de Rama. Moralite et devoir'},
            {name:'Ghrisha',nature:'benefic',desc:'Le radieux. Sagesse et illumination'},
            {name:'Indra',nature:'benefic',desc:'Roi des dieux. Leadership'},
            {name:'Jala',nature:'benefic',desc:'Eau. Flux et adaptation'},
            {name:'Vishwa',nature:'benefic',desc:'Univers. Amour universel'},
            {name:'Amara',nature:'benefic',desc:'Immortel. Poursuite de l\'eternite'},
            {name:'Bala2',nature:'malefic',desc:'Jeune force. Utilisation immature du pouvoir'},
            {name:'Pitri',nature:'malefic',desc:'Ancetres. Karma ancestral'},
            {name:'Rudra',nature:'malefic',desc:'Dieu de la tempete. Transformation destructrice'},
            {name:'Varuna',nature:'benefic',desc:'Dieu de l\'ocean. Ordre cosmique'},
            {name:'Aryama',nature:'benefic',desc:'Divinite solaire. Amitie et contrats'},
            {name:'Mitra',nature:'benefic',desc:'Dieu de l\'amitie. Confiance et camaraderie'},
            {name:'Agni',nature:'malefic',desc:'Dieu du feu. Feu purificateur'},
            {name:'Varuna2',nature:'benefic',desc:'Dieu de l\'ocean. Sagesse profonde'},
            {name:'Gauri',nature:'benefic',desc:'Parvati. Devotion et amour'},
            {name:'Mahakala',nature:'malefic',desc:'Grand Temps. Essayer de maitriser le temps'},
            {name:'Pitamaha',nature:'benefic',desc:'Grand Pere Brahma. Createur'},
            {name:'Kartikeya',nature:'benefic',desc:'Dieu de la guerre. Bataille juste'},
            {name:'Yama',nature:'malefic',desc:'Dieu de la mort. Jugement et justice'},
            {name:'Kala2',nature:'malefic',desc:'Temps. Poursuivi par le temps'},
            {name:'Varuna3',nature:'benefic',desc:'Dieu de l\'ocean. Loi et verite'},
            {name:'Kubera2',nature:'benefic',desc:'Dieu de la richesse. Generosite'},
            {name:'Aditya',nature:'benefic',desc:'Dieu du soleil. Lumiere et verite'},
            {name:'Rishi',nature:'benefic',desc:'Sage. Sagesse et pratique'},
            {name:'Vasu',nature:'benefic',desc:'Etre celeste. Gouverner la nature'},
            {name:'Ashwini',nature:'benefic',desc:'Guerisseurs jumeaux. Guerison'},
            {name:'Naga',nature:'malefic',desc:'Divinite serpent. Mystere et secrets'},
            {name:'Gandharva',nature:'benefic',desc:'Musicien celeste. Art et musique'},
            {name:'Prajapati',nature:'benefic',desc:'Createur. Creer la vie'},
        ...
        if (d60H10planets.length > 0) {
            ch5 += '<br><br><strong>' + (isEasy ? 'Planetes de carriere :' : 'D60 Planets in 10th:') + '</strong> ' + d60H10planets.map(p => p.name).join(', ') + ' — Karma de carriere concentre dans ces planetes.';
        }
        ...
        if (venD2) html += (isEasy ? '' : '<strong>♀ Venus → ' + SIGNS[venD2.dSign] + ':</strong> ') + (isEasy ? (venD2.dSign === 4 ? 'Autodidacte par l\'art/articles de luxe.' : 'Richesse par le conjoint ou partenaire.') : 'Venus in ' + (venD2.dSign === 4 ? 'Sun Hora — self-made through art/luxury goods.' : 'Moon Hora — wealth through spouse or partner.')) + '<br>';
        ...
        if (d3_11planets.length > 0) {
            d3_11planets.forEach(p => { html += isEasy ? 'Influence la relation avec les freres/soeurs aines.<br>' : '• ' + p.name + ' in 11th house — influences relationship with older siblings.<br>'; });
        ...
        if (d16_4planets.length > 0) {
            d16_4planets.forEach(p => {
                html += isEasy ? (p.natural === 'benefic' ? 'Confort materiel et bonheur abondants !<br>' : 'Effort necessaire pour le confort materiel.<br>') : '• ' + p.name + ': ' + (p.natural === 'benefic' ? 'Material comfort and happiness abundant!' : 'Effort needed for material comfort.') + '<br>';
            });
        ...
        if (d20_9planets.length > 0) {
            d20_9planets.forEach(p => { html += isEasy ? 'Forte connexion avec un maitre spirituel.<br>' : '• ' + p.name + ': Strong connection with spiritual teacher.<br>'; });
        ...
        } else html += isEasy ? 'Grandit regulierement dans un environnement educatif stable.<br>' : '4 house no planets.<br>';
        ...
        if (d24_5planets.length > 0) {
            d24_5planets.forEach(p => {
                html += isEasy ? (p.natural === 'benefic' ? 'Reussite exceptionnelle en education superieure !<br>' : 'Les defis academiques menent a la croissance.<br>') : '• ' + p.name + ': ' + (p.natural === 'benefic' ? 'Outstanding higher education achievement!' : 'Academic challenges lead to growth.') + '<br>';
            });
        } else html += isEasy ? 'Un effort regulier apporte de bons resultats.<br>' : '5 house no planets.<br>';
        ...
        if (d27_6planets.length > 0) {
            d27_6planets.forEach(p => { html += isEasy ? 'Attention particuliere necessaire pour cette zone.<br>' : '• ' + p.name + ' in 6th house — special attention needed for this area.<br>'; });
        }
        html += '</div></div>';
        ...
        if (d40_4planets.length > 0) {
            d40_4planets.forEach(p => { html += isEasy ? 'Energie fortement heritee du cote maternel.<br>' : '• ' + p.name + ': This planet\'s energy is strongly inherited from maternal line.<br>'; });
        ...
        if (d45_9planets.length > 0) {
            d45_9planets.forEach(p => { html += isEasy ? 'Energie fortement heritee du cote paternel.<br>' : '• ' + p.name + ': This planet\'s energy is strongly inherited from paternal line.<br>'; });
