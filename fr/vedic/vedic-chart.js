...
function renderD9Interpretation(d9Positions, d9LagnaSign, d1LagnaSign) {
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ...
            ${d9H10Planets.length > 0 ? '<br><br>' + (isEasy ? d9H10Planets.map(p => planetCareer[p.id] || 'unique career energy').join('<br>') : '<strong>Planetes en D9 10e :</strong><br>' + d9H10Planets.map(p => `${p.symbol} <strong>${p.name}</strong>: ${planetCareer[p.id] || 'unique career energy'}`).join('<br>')) : ''}
        </div>
    </div>`;
    ...
    if (division === 10) {
        ...
        if (sunD60) {
            ...
            if (sunD.deity) {
                ch2 += '<br>' + (isEasy ?
                    ...
                    (sunD.deity.nature === 'benefic' ?
                        'Sun deity <strong>' + sunD.deity.ko + '</strong>: ' + sunD.deity.desc + '. But de l\'ame correctement poursuivi — la realisation de soi vient naturellement.' :
                        'Sun deity <strong>' + sunD.deity.ko + '</strong>: ' + sunD.deity.desc + '. Defi des vies passees avec soi/autorite — trouver le vrai soi est la tache de l\'ame.'));
            }
        ...
        if (moonD60) {
            ...
            if (moonD.deity) {
                ch3 += '<br>' + (isEasy ?
                    ...
                    (moonD.deity.nature === 'benefic' ?
                        'Moon deity <strong>' + moonD.deity.ko + '</strong>: ' + moonD.deity.desc + '. L\'esprit etait paisible — emotionnellement stable avec forte intuition.' :
                        'Moon deity <strong>' + moonD.deity.ko + '</strong>: ' + moonD.deity.desc + '. Les blessures emotionnelles restent inconscientes. Reconnaitre et guerir est la tache emotionnelle de cette vie. La meditation et le repos pres de l\'eau aident.'));
            }
        ...
        if (venusD60) {
            ...
            if (isEasy) {
                ...
                ch4 += '<br><strong>♀ Venus (Planete de l\'Amour)</strong> → D60 ' + venH + ' house (' + houseThemes[venH] + ')' + deityTag(venD) + '<br>';
        ...
        if (satD60) {
            ...
            if (!isEasy) ch5 += '<br><strong>♄ Saturne (Seigneur du Karma)</strong> → D60 ' + satH + ' house (' + houseThemes[satH] + ')' + deityTag(satD) + '<br>';
            ch5 += isEasy ?
        ...
        if (jupD20) {
            ...
            html += (isEasy ? '' : '<strong>♃ Jupiter (Maitre Spirituel) → ' + jH + 'th:</strong> ') + ([,'Strong spiritual self','Spiritual knowledge becomes wealth','Spiritual communication ability','Deep inner peace','Past life spiritual merit','Spirituality through service','Meeting a teacher','Secret spiritual knowledge','Best placement! Great spiritual fortune','Spiritual authority','Spiritual community','Liberation and awakening'][jH] || '') + '<br>';
        }
        if (ketuD20) {
        ...
        if (merD24) {
            ...
            html += (isEasy ? '' : '<strong>☿ Mercure (Apprentissage) → ' + mH + 'th:</strong> ') + ([1,4,5,9].includes(mH) ? '📖 <strong>Intellect exceptionnel !</strong> Talent en maths, langues, analyse.' : (isEasy ? 'Capacite intellectuelle bien exprimee.' : 'Intellectual ability in ' + mH + 'th house area.')) + '<br>';
        }
        html += '</div></div>';
        ...
        if (sunD27) {
            ...
            html += (isEasy ? '' : '<strong>☉ Soleil (Vitalite) → ' + sH + 'th:</strong> ') + (isEasy ? 'Source of vitality: ' : 'Source of vitality in ' + sH + 'th house area. ') + ([,'Energy from self','Vitality from wealth activities','Energy from communication','Stability from home','Vitality from creation','Energy from service','Vitality from relationships','Energy from transformation','Vitality from travel','Energy from career','Vitality from society','Energy from spiritual practice'][sH] || '') + '<br>';
        }
