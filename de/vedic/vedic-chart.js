...
function renderD9Interpretation(d9Positions, d9LagnaSign, d1LagnaSign) {
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ${isEasy ? 'Dies offenbart dein wahres Selbst nach der Hochzeit und in der zweiten Lebenshälfte (nach den 30ern).' : 'Navamsa Lagna is in <strong>' + SIGNS[d9LagnaSign] + '</strong>. This reveals your true self after marriage and in the second half of life (after 30s).'}
            ...
            ${d9H1Planets.length > 0 ? '<br><br>' + (isEasy ? 'Es gibt Energien, die deine Persönlichkeit nach der Hochzeit stark beeinflussen.' : '<strong>Planets in D9 1st:</strong> ' + d9H1Planets.map(p => p.symbol + ' ' + p.name).join(', ') + ' — beeinflussen stark deine Persönlichkeit nach der Hochzeit.') : ''}
        </div>
    </div>`;
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💼 Lebenszweck' : '💼 D9 10th House — Life Purpose (Dharma): ' + SIGNS[d9H10Sign] + ' ' + SIGN_SYMBOLS[d9H10Sign]}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '👔 Partnerkarriere' : '👔 Spouse Career — Derived 10th (D9 4th): ' + SIGNS[d9H4Sign] + ' ' + SIGN_SYMBOLS[d9H4Sign]}</div>
        <div class="interp-text">
    ...
    if (vargottamaPlanets.length > 0) {
        html += `<div class="interp-card">
            <div class="interp-title">${isEasy ? '⭐ Außergewöhnlich starke Planeten' : '⭐ Vargottama Planets — Exceptionally Strong'}</div>
            <div class="interp-text">
                ${isEasy ? 'Diese Planeten sind außergewöhnlich kraftvoll und wirken beständig durchs ganze Leben.' : 'Planets in the same sign in both D1 and D9 are called <strong>Vargottama</strong>. Very powerful, acting consistently throughout life.'}<br><br>
...
function renderDivisionalChart(positions, lagnaSidereal, division, chartId, interpId, label, koName) {
    ...
    if (division === 10) {
        ...
        if (isEasy) {
            ...
            html += '<strong>D10 10. Haus (Karriere):</strong> ' + SIGNS[d10_10sign] + ' (ruler: ' + (RULER_NAMES[d10_10lord]||d10_10lord) + ')<br>';
        }
        ...
        if (!isEasy) {
            ...
            html += '<strong>D7 5. Haus (Kinder):</strong> ' + SIGNS[d7_5sign] + ' (ruler: ' + (RULER_NAMES[d7_5lord]||d7_5lord) + ')<br>';
        }
        ...
        const D60_DEITIES = [
            {name:'Ghora',nature:'malefic',desc:'Zerstörung und Furcht. Dunkles Karma aus früheren Leben'},
            {name:'Rakshasa',nature:'malefic',desc:'Dämonische Energie. Starkes Verlangen und Anhaftung'},
            {name:'Deva',nature:'benefic',desc:'Göttliches Wesen. Verdienst und Segen aus früheren Leben'},
            {name:'Kubera',nature:'benefic',desc:'Gott des Reichtums. Wohlstandsaufbau-Karma'},
            {name:'Yaksha',nature:'benefic',desc:'Naturwächter. Harmonie mit der Natur'},
            {name:'Kinnara',nature:'benefic',desc:'Himmlischer Musiker. Künstlerisches Talent'},
            {name:'Bhrashta',nature:'malefic',desc:'Der Gefallene. Karma des Fallens aus der Höhe'},
            {name:'Kulaghna',nature:'malefic',desc:'Familienzerstörer. Familienbezogenes Karma'},
            {name:'Garala',nature:'malefic',desc:'Gift. Karma toxischer Handlungen'},
            {name:'Vahni',nature:'malefic',desc:'Feuergott. Karma von Zorn und Zerstörung'},
            {name:'Maya',nature:'malefic',desc:'Illusion. Karma der Täuschung'},
            {name:'Purishaka',nature:'malefic',desc:'Knechtschaft. Karma der Einschränkung anderer'},
            {name:'Apampathi',nature:'benefic',desc:'Herr der Gewässer. Reinigung und Heilung'},
            {name:'Marut',nature:'benefic',desc:'Windgott. Freiheit und Veränderung'},
            {name:'Kala',nature:'malefic',desc:'Gott der Zeit. Zeit- und Todeskarma'},
            {name:'Sarpa',nature:'malefic',desc:'Schlange. Bindung und Anhaftung — unfähig loszulassen'},
            {name:'Amrita',nature:'benefic',desc:'Nektar der Unsterblichkeit. Streben nach ewigem Leben'},
            {name:'Indu',nature:'benefic',desc:'Mond. Sensibilität und Intuition'},
            {name:'Mridu',nature:'benefic',desc:'Der Sanfte. Sanftmut und Mitgefühl'},
            {name:'Komala',nature:'benefic',desc:'Der Zarte. Kunst und Schönheit'},
            {name:'Heramba',nature:'benefic',desc:'Ganesha-Avatar. Hindernisse überwinden'},
            {name:'Brahma',nature:'benefic',desc:'Schöpfergott. Schöpfung und Wissen'},
            {name:'Vishnu',nature:'benefic',desc:'Bewahrergott. Schutz und Ordnung'},
            {name:'Maheshwara',nature:'benefic',desc:'Großer Herr Shiva. Transformation und Befreiung'},
            {name:'Deva2',nature:'benefic',desc:'Heiliger. Spirituelle Praxis'},
            {name:'Bala',nature:'benefic',desc:'Stärke. Tapferkeit und Mut'},
            {name:'Vishwakarma',nature:'benefic',desc:'Kosmischer Architekt. Bauen und Erschaffen'},
            {name:'Tamasa',nature:'malefic',desc:'Dunkelheit. Karma der Unwissenheit'},
            {name:'Kanchana',nature:'benefic',desc:'Gold. Reinheit und Wert'},
            {name:'Varaha',nature:'benefic',desc:'Vishnu-Eber-Avatar. Erlösung'},
            {name:'Ramasala',nature:'benefic',desc:'Wohnstätte Ramas. Moral und Pflicht'},
            {name:'Ghrisha',nature:'benefic',desc:'Der Strahlende. Weisheit und Erleuchtung'},
            {name:'Indra',nature:'benefic',desc:'König der Götter. Führung'},
            {name:'Jala',nature:'benefic',desc:'Wasser. Fließen und Anpassung'},
            {name:'Vishwa',nature:'benefic',desc:'Universum. Universelle Liebe'},
            {name:'Amara',nature:'benefic',desc:'Unsterblich. Streben nach Ewigkeit'},
            {name:'Bala2',nature:'malefic',desc:'Junge Kraft. Unreifer Machtgebrauch'},
            {name:'Pitri',nature:'malefic',desc:'Ahnen. Ahnenkarma'},
            {name:'Rudra',nature:'malefic',desc:'Sturmgott. Zerstörerische Transformation'},
            {name:'Varuna',nature:'benefic',desc:'Ozeangott. Kosmische Ordnung'},
            {name:'Aryama',nature:'benefic',desc:'Sonnengottheit. Freundschaft und Verträge'},
            {name:'Mitra',nature:'benefic',desc:'Gott der Freundschaft. Vertrauen und Kameradschaft'},
            {name:'Agni',nature:'malefic',desc:'Feuergott. Reinigendes Feuer'},
            {name:'Varuna2',nature:'benefic',desc:'Ozeangott. Tiefe Weisheit'},
            {name:'Gauri',nature:'benefic',desc:'Parvati. Hingabe und Liebe'},
            {name:'Mahakala',nature:'malefic',desc:'Große Zeit. Versuch die Zeit zu meistern'},
            {name:'Pitamaha',nature:'benefic',desc:'Großer Vater Brahma. Schöpfer'},
            {name:'Kartikeya',nature:'benefic',desc:'Kriegsgott. Gerechter Kampf'},
            {name:'Yama',nature:'malefic',desc:'Totengott. Urteil und Gerechtigkeit'},
            {name:'Kala2',nature:'malefic',desc:'Zeit. Von der Zeit verfolgt'},
            {name:'Varuna3',nature:'benefic',desc:'Ozeangott. Gesetz und Wahrheit'},
            {name:'Kubera2',nature:'benefic',desc:'Gott des Reichtums. Großzügigkeit'},
            {name:'Aditya',nature:'benefic',desc:'Sonnengott. Licht und Wahrheit'},
            {name:'Rishi',nature:'benefic',desc:'Weiser. Weisheit und Praxis'},
            {name:'Vasu',nature:'benefic',desc:'Himmlisches Wesen. Naturbeherrschung'},
            {name:'Ashwini',nature:'benefic',desc:'Zwillingsheiler. Heilung'},
            {name:'Naga',nature:'malefic',desc:'Schlangengottheit. Geheimnis und Geheimnisse'},
            {name:'Gandharva',nature:'benefic',desc:'Himmlischer Musiker. Kunst und Musik'},
            {name:'Prajapati',nature:'benefic',desc:'Schöpfer. Leben erschaffen'},
            {name:'Charachara',nature:'benefic',desc:'Alle Dinge. Eins mit allem'}
        ];
        ...
        if (!isEasy) {
            ...
            html += '📜 <strong>Parasara sagt:</strong> "Shashtiamsha (D60) is the most important of all divisional charts. Benefic planets in benefic divisions give good results, malefic planets in malefic divisions give bad results."<br>';
        ...
        if (h7lordPlanet) {
            ...
            if (isEasy) {
                ...
                ch4 += h7lH === 1 ? 'Partner direkt mit deinem Wachstum verbunden.' : h7lH === 4 ? 'Treffe Partner durch Zuhause und Zufluchtsort.' : h7lH === 9 ? 'Partnerverbindung durch Ausland/Bildung.' : h7lH === 10 ? 'Partnerverbindung durch Karriere/Soziales.' : h7lH === 12 ? 'Karma to meet spouse in foreign/spiritual settings.' : '';
            }
        }
        ...
        if (d60H10planets.length > 0) {
            ch5 += '<br><br><strong>' + (isEasy ? 'Karriereplaneten:' : 'D60 Planets in 10th:') + '</strong> ' + d60H10planets.map(p => p.name).join(', ') + ' — Karriere-Karma in diesen Planeten konzentriert.';
        }
        ...
        if (jupD2) html += (isEasy ? '' : '<strong>♃ Jupiter → ' + SIGNS[jupD2.dSign] + ':</strong> ') + (isEasy ? (jupD2.dSign === 4 ? 'Kann großen Wohlstand durch eigene Fähigkeit aufbauen.' : 'Überfluss durch Beziehungen mit anderen.') : 'Jupiter in ' + (jupD2.dSign === 4 ? 'Sun Hora — great wealth through own ability.' : 'Moon Hora — abundance through relationships with others.')) + '<br>';
        if (venD2) html += (isEasy ? '' : '<strong>♀ Venus → ' + SIGNS[venD2.dSign] + ':</strong> ') + (isEasy ? (venD2.dSign === 4 ? 'Selbstgemacht durch Kunst/Luxusgüter.' : 'Wohlstand durch Ehepartner oder Partner.') : 'Venus in ' + (venD2.dSign === 4 ? 'Sun Hora — self-made through art/luxury goods.' : 'Moon Hora — wealth through spouse or partner.')) + '<br>';

        // D2 2궁(축적된 부) 분석
        const d2H2sign = (dLagnaSign + 1) % 12;
        const d2H2planets = dPositions.filter(p => p.dSign === d2H2sign);
        html += '<br><strong>' + (isEasy ? 'Angesammelter Wohlstand:' : 'D2 2nd house (accumulated wealth) — ' + SIGNS[d2H2sign] + ':') + '</strong><br>';
        ...
        html += '<strong>' + (isEasy ? 'Jüngere Geschwister:' : 'D3 3 house (younger) — ' + SIGNS[d3_3sign] + ':') + '</strong><br>';
        ...
        html += '<br><strong>' + (isEasy ? 'Ältere Geschwister:' : 'D3 11th house (older siblings) — ' + SIGNS[d3_11sign] + ':') + '</strong><br>';
        if (d3_11planets.length > 0) {
            d3_11planets.forEach(p => { html += isEasy ? 'Beeinflusst Beziehung zu älteren Geschwistern.<br>' : '• ' + p.name + ' in 11th house — influences relationship with older siblings.<br>'; });
        ...
        html += '<br><strong>' + (isEasy ? 'Allgemeines Glück:' : 'D4 10th house (overall fortune) — ' + SIGNS[d4_10sign] + ':') + '</strong><br>';
        ...
        html += '<strong>' + (isEasy ? 'Komfort/Glück:' : 'D16 4th house (comfort/happiness) — ' + SIGNS[d16_4sign] + ':') + '</strong><br>';
        if (d16_4planets.length > 0) {
            d16_4planets.forEach(p => {
                html += isEasy ? (p.natural === 'benefic' ? 'Reichlicher materieller Komfort und Glück!<br>' : 'Einsatz nötig für materiellen Komfort.<br>') : '• ' + p.name + ': ' + (p.natural === 'benefic' ? 'Material comfort and happiness abundant!' : 'Effort needed for material comfort.') + '<br>';
            });
        ...
        html += '<br><strong>' + (isEasy ? 'Guru/Lehrer:' : 'D20 9th house (guru/teacher) — ' + SIGNS[d20_9sign] + ':') + '</strong><br>';
        if (d20_9planets.length > 0) {
            d20_9planets.forEach(p => { html += isEasy ? 'Starke Verbindung zu spirituellem Lehrer.<br>' : '• ' + p.name + ': Strong connection with spiritual teacher.<br>'; });
        ...
        html += '<br><strong>' + (isEasy ? 'Höhere Bildung:' : 'D24 5th house (higher education/intellect) — ' + SIGNS[d24_5sign] + ':') + '</strong><br>';
        if (d24_5planets.length > 0) {
            d24_5planets.forEach(p => {
                html += isEasy ? (p.natural === 'benefic' ? 'Herausragende Leistung in höherer Bildung!<br>' : 'Akademische Herausforderungen führen zu Wachstum.<br>') : '• ' + p.name + ': ' + (p.natural === 'benefic' ? 'Outstanding higher education achievement!' : 'Academic challenges lead to growth.') + '<br>';
            });
        } else html += isEasy ? 'Stetiger Einsatz bringt gute Ergebnisse.<br>' : '5 house no planets.<br>';

        if (jupD24) {
            const jH = ((jupD24.dSign - dLagnaSign + 12) % 12) + 1;
            html += (isEasy ? '<br>' : '<br><strong>♃ Jupiter (Wisdom) → ' + jH + 'th:</strong> ') + ([1,4,5,9].includes(jH) ? '🎓 <strong>Hohe akademische Leistung erwartet!</strong> Postgraduale/Promotion/Auslandsstudium möglich.' : (isEasy ? 'Wachstum durch Akademik erwartet.' : 'Growth through academics. Jupiter\'s blessing manifests in ' + jH + 'th house area.')) + '<br>';
        }
        if (merD24) {
            const mH = ((merD24.dSign - dLagnaSign + 12) % 12) + 1;
            html += (isEasy ? '' : '<strong>☿ Mercury (Learning) → ' + mH + 'th:</strong> ') + ([1,4,5,9].includes(mH) ? '📖 <strong>Herausragender Intellekt!</strong> Talent in Mathe, Sprachen, Analyse.' : (isEasy ? 'Intellektuelle Fähigkeit gut ausgedrückt.' : 'Intellectual ability in ' + mH + 'th house area.')) + '<br>';
        }
        html += '</div></div>';
        ...
        html += '<br><strong>' + (isEasy ? 'Schwäche:' : 'D27 6th house (weakness/vulnerability) — ' + SIGNS[d27_6sign] + ':') + '</strong><br>';
        ...
        if (d27_6planets.length > 0) {
            d27_6planets.forEach(p => { html += isEasy ? 'Besondere Aufmerksamkeit für diesen Bereich nötig.<br>' : '• ' + p.name + ' in 6th house — special attention needed for this area.<br>'; });
        }
        html += '</div></div>';
        ...
        html += '<strong>' + (isEasy ? 'Krankheitswarnung:' : 'D30 6th house (disease/enemy) — ' + SIGNS[d30_6sign] + ':') + '</strong><br>';
        ...
        html += '<br><strong>' + (isEasy ? 'Gefahr/Operation:' : 'D30 8 house (danger/surgery) — ' + SIGNS[d30_8sign] + ':') + '</strong><br>';
        ...
        html += '<br><strong>' + (isEasy ? 'Krankenhausaufenthalt:' : 'D30 12th house (hospitalization/loss) — ' + SIGNS[d30_12sign] + ':') + '</strong><br>';
        ...
        html += '<br><strong>' + (isEasy ? 'Mütterliche Familie:' : 'D40 4th house (maternal home) — ' + SIGNS[d40_4sign] + ':') + '</strong><br>';
        if (d40_4planets.length > 0) {
            d40_4planets.forEach(p => { html += isEasy ? 'Stark geerbte Energie von mütterlicher Seite.<br>' : '• ' + p.name + ': This planet\'s energy is strongly inherited from maternal line.<br>'; });
        ...
        html += '<br><strong>' + (isEasy ? 'Väterliche Familie:' : 'D45 9th house (paternal home/father) — ' + SIGNS[d45_9sign] + ':') + '</strong><br>';
        if (d45_9planets.length > 0) {
            d45_9planets.forEach(p => { html += isEasy ? 'Stark geerbte Energie von väterlicher Seite.<br>' : '• ' + p.name + ': This planet\'s energy is strongly inherited from paternal line.<br>'; });
