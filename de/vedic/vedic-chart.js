...
function renderD9Interpretation(d9Positions, d9LagnaSign, d1LagnaSign) {
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ...
            ${d9H1Planets.length > 0 ? '<br><br>' + (isEasy ? 'Es gibt Energien, die deine Persönlichkeit nach der Hochzeit stark beeinflussen.' : '<strong>Planets in D9 1st:</strong> ' + d9H1Planets.map(p => p.symbol + ' ' + p.name).join(', ') + ' — strongly influence your personality after marriage.') : ''}
        </div>
    </div>`;
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ${isEasy ? 'Analyse, aus welcher Richtung dein Partner kommen könnte.' : 'Vedic astrology determines spouse direction by combining multiple indicators.'}<br><br>
...
function renderInterpretation(positions, lagnaSign, moonPos) {
    ...
    if (sun && mercury && sun.sign === mercury.sign) {
        yogaText += isEasy ? '<strong>📚 Segen herausragender Intelligenz</strong>' : '<strong>📚 Budha-Aditya Yoga</strong> — Sun-Mercury same sign! Outstanding intellect and communication. Success in education, writing, business. Authoritative intellectual leader.<br><br>';
    }
...
function renderLucky(lagnaSign, moonPos) {
    const luckyData = [
        { color: 'Red, Orange', number: '1, 9', day: 'Dienstag', gem: 'Red Coral', dir: 'East' },
        { color: 'White, Pink', number: '2, 6', day: 'Freitag', gem: 'Diamond', dir: 'Southeast' },
        { color: 'Green', number: '3, 5', day: 'Mittwoch', gem: 'Emerald', dir: 'North' },
        { color: 'White, Silver', number: '2, 7', day: 'Montag', gem: 'Pearl', dir: 'Northwest' },
        { color: 'Gold, Orange', number: '1, 4', day: 'Sonntag', gem: 'Ruby', dir: 'East' },
        { color: 'Green, Light Green', number: '5, 3', day: 'Mittwoch', gem: 'Emerald', dir: 'South' },
        { color: 'White, Pastel', number: '6, 2', day: 'Freitag', gem: 'Diamond', dir: 'West' },
        { color: 'Red, Crimson', number: '9, 1', day: 'Dienstag', gem: 'Red Coral', dir: 'South' },
        { color: 'Yellow, Gold', number: '3, 9', day: 'Donnerstag', gem: 'Yellow Sapphire', dir: 'Northeast' },
        { color: 'Navy, Black', number: '8, 4', day: 'Samstag', gem: 'Blue Sapphire', dir: 'West' },
        { color: 'Navy, Purple', number: '4, 8', day: 'Samstag', gem: 'Blue Sapphire', dir: 'West' },
        { color: 'Yellow, Gold', number: '3, 7', day: 'Donnerstag', gem: 'Yellow Sapphire', dir: 'Northeast' }
    ];
...
function renderDivisionalChart(positions, lagnaSidereal, division, chartId, interpId, label, koName) {
    ...
    if (division === 10) {
        ...
        if (benefics.length > 0) html += (isEasy ? 'Wohltätige Planeten — mit Kindern gesegnet.' : 'Benefic planets in 5th house — blessed with children.') + '<br>';
        if (malefics.length > 0) html += (isEasy ? 'Herausfordernde Planeten — Schwierigkeiten mit Kindern möglich.' : 'Malefic planets in 5th house — children related difficulties possible.') + '<br>';
        if (d7_5planets.length === 0) html += isEasy ? 'Keine Planeten in Kinderposition — andere Faktoren müssen analysiert werden.' : '5th house empty — check 5th lord position.';
        html += '</div></div>';
        ...
        if (sunD2) {
            ...
            html += (isEasy ? '' : '<strong>☉ Sun → ' + SIGNS[sunD2.dSign] + ':</strong> ') + (sunInOwn ? (isEasy ? '🌟 <strong>Selbstgemacht!</strong> Baut Wohlstand durch Autorität und Führung auf.' : '🌟 <strong>Sun in own Hora (Leo)!</strong> Self-made type. Builds wealth through authority and leadership.') : (isEasy ? 'Einkommen durch andere oder Regierung/öffentlichen Sektor.' : 'Sun in Moon Hora. Income through others help or government/public sector.')) + '<br>';
        }
        if (moonD2) {
            const moonInOwn = moonD2.dSign === 3; // Cancer
            html += (isEasy ? '' : '<strong>☽ Moon → ' + SIGNS[moonD2.dSign] + ':</strong> ') + (moonInOwn ? (isEasy ? '🌟 <strong>Reiches Leben durch öffentliche Beziehungen!</strong>' : '🌟 <strong>Moon in own Hora (Cancer)!</strong> Abundant life through public and relationships.') : (isEasy ? 'Einkommen durch eigene Anstrengung und unabhängige Aktivität.' : 'Moon in Sun Hora. Income through own effort and independent activity.')) + '<br>';
        }
        ...
        if (d4_10planets.length > 0) {
            d4_10planets.forEach(p => {
                html += isEasy ? (p.natural === 'benefic' ? 'Allgemeines Glück ist gut!<br>' : 'Einsatz nötig aber Wachstumschance.<br>') : '• ' + p.name + ': ' + (p.natural === 'benefic' ? 'Benefic in 10th house — overall fortune is good!' : 'Malefic in 10th house — effort needed for fortune but opportunity for growth.') + '<br>';
            });
        ...
        } else html += isEasy ? 'Durchschnittlicher materieller Komfort.<br>' : '4th house empty — 4th lord position is key to happiness.<br>';
        ...
        } else html += isEasy ? 'Gut, aktiv einen spirituellen Lehrer zu suchen.<br>' : '9th house empty — actively seek a spiritual teacher.<br>';
        html += '</div></div>';
        ...
        } else html += isEasy ? 'Stabiles Erbe von mütterlicher Seite.<br>' : '4th house empty — 4th lord position is key to maternal heritage.<br>';
        html += '</div></div>';
        ...
        } else html += isEasy ? 'Stabiles Erbe von väterlicher Seite.<br>' : '9th house empty — 9th lord position is key to paternal heritage.<br>';
        html += '</div></div>';
    }
