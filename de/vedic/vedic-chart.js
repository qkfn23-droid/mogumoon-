    ...
    if (moonPos) {
        ...
        if (nak) {
            ...
            for (var i=0;i<9;i++) {
                ...
                if (now>=cd && now<endD) {
                    var dashaDesc = {Ketu:'Eine Phase spirituellen Wachstums. Konzentriere dich auf dein Inneres statt auf Materielles.',Venus:'Eine Zeit der Liebe und des Überflusses! Romantik, Ehe und Kunst blühen auf.',Sun:'Eine Zeit der Selbstfindung und Führung. Das Selbstvertrauen wächst stark.',Moon:'Eine Zeit der Emotionen und des Zuhauses. Familienbeziehungen werden wichtig.',Mars:'Eine Zeit der Aktion und Energie. Großartig für neue Anfänge.',Rahu:'Eine Zeit des Wandels und der Innovation. Unerwartete Möglichkeiten entstehen.',Jupiter:'Eine Zeit des Glücks und Wachstums! Viele gute Dinge in Bildung, Ehe, Beförderung.',Saturn:'Eine Zeit der Geduld und Prüfungen. Langsames aber sicheres Wachstum.',Mercury:'Eine Zeit intellektueller Aktivität. Günstig für Studium, Geschäft, Kommunikation.'};
                    html += '<div class="interp-card"><div class="interp-title">⏳ Meine aktuelle Periode</div><div class="interp-text">Current period: <strong style="color:#c9a84c;">' + DASHA_KO[planet] + '</strong> period.<br><br>' + (dashaDesc[planet]||'') + '</div></div>';
                    break;
                }
                cd = endD;
            }
...
function renderSpouseProfile(d1LagnaSign, ulSign, a7Sign, venusD9Sign) {
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🤝 Begegnungsumgebung' : '🤝 Meeting Environment — D1 7th: ' + SIGNS[d1H7ForMeeting] + ' ' + SIGN_SYMBOLS[d1H7ForMeeting]}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🏛️ Familienhintergrund des Partners' : '🏛️ Spouse Background — UL: ' + SIGNS[ulSign] + ' ' + SIGN_SYMBOLS[ulSign]}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '👤 Erster Eindruck des Partners' : '👤 Spouse First Impression — A7: ' + SIGNS[a7Sign] + ' ' + SIGN_SYMBOLS[a7Sign]}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💎 Anziehungspunkt des Partners' : '💎 Spouse Attraction — D9 Venus: ' + SIGNS[venusD9Sign] + ' ' + SIGN_SYMBOLS[venusD9Sign]}</div>
        <div class="interp-text">
...
function renderInterpretation(positions, lagnaSign, moonPos) {
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💍 Mein Partner' : '💕 Spouse & Marriage — 7th House: ' + SIGNS[h7sign] + ' ' + SIGN_SYMBOLS[h7sign]}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💼 Meine Karriere' : '💼 career & societyAchievement — 10th: ' + SIGNS[h10sign] + ' ' + SIGN_SYMBOLS[h10sign]}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🏥 Meine Gesundheit' : '🏥 health — Vulnerable Areas'}</div>
