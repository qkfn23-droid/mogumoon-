...
function updateCatHeaders() {
    ...
    var ids = {
        catGuide: e ? 'Glossar' : 'Vedische Astrologie Guide',
        catBasic: e ? 'Meine Planetenpositionen' : 'Grundkarte — Planetenpositionen & Geburtskarte',
        catDasha: e ? 'Meine Lebensphasen' : 'Dasha — Lebensphasen',
        catInterp: e ? 'Meine Deutung — Persönlichkeit·Wohlstand·Karriere·Gesundheit' : 'Interpretation — Persönlichkeit·Wohlstand·Karriere·Gesundheit·Yoga',
        catMarriage: e ? 'Mein Partner' : 'Ehe & Partner — D9 Navamsha',
        ...
        catWarn: e ? 'Gesundheitshinweise' : 'Warnungen — D30 Krankheit·Ausland',
        catKarma: e ? 'Vergangenes Leben Karma' : 'Karma — D60 Vergangenes Leben',
    };
...
function renderD9Interpretation(d9Positions, d9LagnaSign, d1LagnaSign) {
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🕉️ Sie nach der Ehe: ${SIGNS[d9LagnaSign]} ${SIGN_SYMBOLS[d9LagnaSign]}' : '🕉️ D9 Lagna — Sie nach der Ehe: ${SIGNS[d9LagnaSign]} ${SIGN_SYMBOLS[d9LagnaSign]}'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💍 Charakter des Ehepartners: ${SIGNS[d9H7Sign]} ${SIGN_SYMBOLS[d9H7Sign]}' : '💍 D9 7. Haus — Charakter des Ehepartners: ${SIGNS[d9H7Sign]} ${SIGN_SYMBOLS[d9H7Sign]}'}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💼 Lebensaufgabe (Dharma): ${SIGNS[d9H10Sign]} ${SIGN_SYMBOLS[d9H10Sign]}' : '💼 D9 10. Haus — Lebensaufgabe (Dharma): ${SIGNS[d9H10Sign]} ${SIGN_SYMBOLS[d9H10Sign]}'}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '👔 Karriere des Ehepartners' : '👔 Karriere des Ehepartners — Abgeleitetes 10. Haus (D9 4. Haus): ${SIGNS[d9H4Sign]} ${SIGN_SYMBOLS[d9H4Sign]}'}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🧭 Spouse Direction' : '🧭 Richtung des Ehepartners — 6-Indikator-Analyse'}</div>
        <div class="interp-text">
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '👤 Erster Eindruck des Ehepartners' : '👤 Erster Eindruck des Ehepartners — A7: ${SIGNS[a7Sign]} ${SIGN_SYMBOLS[a7Sign]}'}</div>
        <div class="interp-text">
...
function renderInterpretation(positions, lagnaSign, moonPos) {
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '👤 Persönlichkeit & Erscheinung' : '👤 Persönlichkeit & Erscheinung — Lagna: ${SIGNS[lagnaSign]} ${SIGN_SYMBOLS[lagnaSign]}'}</div>
    ...
    if (moonPos) {
        const moonEasy = [
        'Eine feurige Leidenschaft brennt in dir. Emotionen steigen schnell und fallen schnell. Bei Stress musst du dich bewegen — Sport hilft am besten.',
        'Emotional sehr stabil. Du magst keine plötzlichen Veränderungen. Gutes Essen, Musik und Natur heilen dich. Einmal gegeben, ändert sich dein Herz selten.',
        'Du verarbeitest Emotionen durch Gespräche. Reden hilft dir, dich besser zu fühlen. Neugierig und kannst Langeweile nicht ertragen.',
        'Extrem sensibel und empathisch. Du absorbierst die Emotionen anderer wie ein Schwamm. Zuhause ist dein sicherer Hafen. Kochen oder Dekorieren bringt emotionalen Frieden.',
        'Dramatischer und leidenschaftlicher emotionaler Ausdruck. Du brauchst Liebe und Anerkennung. Aber du gibst Liebe genauso großzügig. Kreative Aktivitäten sind deine Medizin.',
        'Du analysierst und ordnest deine Emotionen. Du sorgst dich viel, aber löst Probleme praktisch. Tägliche Routinen bringen emotionale Stabilität.',
        'Du findest emotionales Gleichgewicht in Beziehungen. Allein fühlst du dich einsam. Du hasst Konflikte zutiefst. Kunst und Schönheit geben dir Frieden.',
        'Deine Emotionen sind so tief und intensiv wie der Ozean. Du liebst tief und vergisst Verrat nie. Deine Intuition ist unglaublich stark.',
        'Emotional hell und optimistisch. Du liebst Freiheit und hasst Einschränkungen. Reisen ist dein bestes emotionales Heilmittel.',
        'Du zeigst Emotionen nicht leicht. Starkes Pflichtbewusstsein. Mit dem Alter wirst du emotional offener.',
        'Einzigartige, unvorhersehbare emotionale Muster. Du liebst auf unkonventionelle Weise. Soziale Aktivitäten geben dir emotionale Erfüllung.',
        'Extrem intuitiv und spirituell. Lebhafte Träume, die manchmal prophetisch wirken. Kunst, Meditation und Wasser geben dir Frieden.'
    ];
        const moonInterp = [
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💰 Mein Wohlstand' : '💰 Reichtums-Schicksal'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💍 Mein Partner' : '💕 Ehepartner & Ehe-Schicksal — 7. Haus: ${SIGNS[h7sign]} ${SIGN_SYMBOLS[h7sign]}'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💼 Meine Karriere' : '💼 Karriere & Sozialer Erfolg — 10. Haus: ${SIGNS[h10sign]} ${SIGN_SYMBOLS[h10sign]}'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🏥 Meine Gesundheit' : '🏥 Gesundheit — Gefährdete Bereiche'}</div>
    ...
    if (moonPos && jupiter) {
        ...
        if (diff === 0 || diff === 3 || diff === 6 || diff === 9) {
            yogaText += isEasy ? '<strong>📚 Segen außergewöhnlicher Intelligenz</strong>' : '<strong>🐘 Gajakesari Yoga</strong> — Moon and Jupiter in Kendra relationship! A combination of wisdom, fame, and abundance. Socially respected with exceptional intellectual abilities. Good education and children fortune.<br><br>';
        }
    }
    ...
    if (sun && mercury && sun.sign === mercury.sign) {
        yogaText += isEasy ? '<strong>📚 Segen außergewöhnlicher Intelligenz</strong>' : '<strong>📚 Budha-Aditya Yoga</strong> — Sun and Mercury in the same sign! Outstanding intellect and communication skills. Success in education, writing, and business. An authoritative intellectual leader.<br><br>';
    }
...
function renderForeign(positions, lagnaSign) {
    ...
    if (h9.length === 0) {
        text += isEasy ? '' : 'Keine Planeten im 9. Haus — Auslandsreisen gibt es, aber keine besonders starke Verbindung.';
    } else {
        h9.forEach(p => {
    ...
    if (h12.length === 0) {
        text += isEasy ? '' : 'Keine Planeten im 12. Haus — Inlandsresidenz ist natuerlicher als Auslandsniederlassung.';
    } else {
        h12.forEach(p => {
...
function renderDivisionalChart(positions, lagnaSidereal, division, chartId, interpId, label, koName) {
    ...
    if (division === 10) {
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '💼 Karriereanalyse' : '💼 D10 Karriereanalyse') + '</div><div class="interp-text">';
        ...
        if (benefics.length > 0) html += 'Wohltaeter im 5. — mit Kindern gesegnet.<br>';
        if (malefics.length > 0) html += 'Uebeltaeter im 5. — Herausforderungen mit Kindern.<br>';
        if (d7_5planets.length === 0) html += isEasy ? '' : 'Keine Planeten im 5. — Position des Herrschers pruefen.';
        html += '</div></div>';
        ...
        html += '<strong>D12 4. (Mutter):</strong> ' + SIGNS[d12_4sign];
        if (d12_4planets.length > 0) html += ' — ' + d12_4planets.map(p => p.name).join(', ');
        html += '<br>';
        html += '<strong>D12 9. (Vater):</strong> ' + SIGNS[d12_9sign];
        ...
        if (moon4) html += 'Mond im 4. — tiefe Bindung zur Mutter.<br>';
        if (sun9) html += 'Sonne im 9. — tiefe Bindung zum Vater.<br>';
        html += '</div></div>';
        ...
        const D60_DEITIES = [
            {name:'Ghora',nature:'malefic',desc:'Zerstoerung und Angst. Dunkles Karma'},
            {name:'Rakshasa',nature:'malefic',desc:'Daemonische Energie. Starkes Verlangen'},
            {name:'Deva',nature:'benefic',desc:'Goettliches Wesen. Verdienst und Segen aus frueheren Leben'},
            {name:'Kubera',nature:'benefic',desc:'Gott des Reichtums. Wohlstandsaufbau'},
            {name:'Yaksha',nature:'benefic',desc:'Naturwaechter. Harmonie mit der Natur'},
            {name:'Kinnara',nature:'benefic',desc:'Himmlischer Musiker. Kuenstlerisches Talent'},
            {name:'Bhrashta',nature:'malefic',desc:'Der Gefallene. Karma des Falls'},
            {name:'Kulaghna',nature:'malefic',desc:'Familienzerstoerer. Familienkarma'},
            {name:'Garala',nature:'malefic',desc:'Gift. Karma giftiger Handlungen'},
            {name:'Vahni',nature:'malefic',desc:'Feuergott. Karma von Wut und Zerstoerung'},
            {name:'Maya',nature:'malefic',desc:'Illusion. Karma der Taeuschung'},
            ...
            {name:'Brahma',nature:'benefic',desc:'Schoepfergott. Schoepfung und Wissen'},
            {name:'Vishnu',nature:'benefic',desc:'Bewahrergott. Schutz und Ordnung'},
            {name:'Maheshwara',nature:'benefic',desc:'Grosser Herr Shiva. Transformation und Befreiung'},
            {name:'Deva2',nature:'benefic',desc:'Heiliger. Spirituelle Praxis'},
            ...
            {name:'Kanchana',nature:'benefic',desc:'Gold. Reinheit und Wert'},
            ...
            {name:'Aryama',nature:'benefic',desc:'Sonnen-Gottheit. Friendship and contracts'},
            ...
            {name:'Varuna2',nature:'benefic',desc:'Meeresgott. Tiefe Weisheit'},
            {name:'Gauri',nature:'benefic',desc:'Parvati. Hingabe und Liebe'},
            ...
            {name:'Yama',nature:'malefic',desc:'Todesgott. Urteil und Gerechtigkeit'},
            ...
            {name:'Charachara',nature:'benefic',desc:'Alle Dinge. Einheit mit allem'}
        ];
        ...
        const pastLifeThemes = [
            'Krieger, Fuehrer — Uebte Macht aus, natuerliche Fuehrung und Entschlossenheit in der Seele.',
            'Kuenstler, Bauer — Arbeitete mit der Natur, tiefer Instinkt fuer Stabilitaet und materielle Schoenheit.',
            'Gelehrter, Haendler — Lebte von Wissen, Vielseitigkeit und Neugier bleiben.',
            'Beschuetzer, Betreuer — Kueimmerte sich um andere, tiefe Sensibilitaet und muetterlicher Instinkt.',
            'Koeniglichkeit, Priester — Hatte hohen Status, natuerliche Autoritaet und Wuerde.',
            'Heiler, Diener — Praktizierte Medizin oder Dienst, ausgezeichnete analytische Faehigkeiten.',
            'Diplomat, Kuenstler — Verfolgte Harmonie und Schoenheit, geschickt in Beziehungen.',
            'Praktiker, Alchemist — Durchlief tiefe Transformation, starke Anziehung zu Geheimnissen.',
            'Weiser, Entdecker — Suchte Wahrheit, spirituelle Weisheit und Abenteuerlust bleiben.',
            'Beamter, Architekt — Baute Ordnung, starke Geduld und Verantwortung.',
            'Beamter, Waechter — Baute soziale Ordnung, Organisationsgeist. Saturn-regiert.',
            'Medium, Kuenstler — Kommunizierte mit der geistigen Welt, aeusserst starke Intuition.'
        ];
        ...
        html += '📜 <strong>Parashara sagte:</strong> "Shashtiamsa (D60) ist die wichtigste aller Teilungscharts. Planeten in wohltaetigen Goetterbereichen geben gute Ergebnisse, uebeltaetige schlechte."<br>';
        ...
        if (lagnaD.deity) {
            ch1 += '<br>' + (isEasy ?
                (lagnaD.deity.nature === 'benefic' ? 'Du hast in früheren Leben viel Gutes getan, gute Gelegenheiten kommen natürlich. Deine Existenz ist geschützt.' : 'Es gibt ungelöste Lektionen aus früheren Leben. Sie beeinflussen dich, aber ihre Überwindung führt zu Wachstum.') :
                (lagnaD.deity.nature === 'benefic' ?
                    '<strong>' + lagnaD.deity.name + '</strong> schuetzt den Lagna. ' + lagnaD.deity.desc + ' — Verdienste aus frueheren Leben schuetzen dieses Leben, gute Gelegenheiten kommen natuerlich.' :
                    '<strong>' + lagnaD.deity.name + '</strong> beeinflusst den Lagna. ' + lagnaD.deity.desc + ' — Karmische Herausforderung in der Persoenlichkeit, aber Ueberwindung fuehrt zu grösserem Wachstum.'));
        }
        if (d60_planets_1.length > 0) ch1 += '<br><br>' + d60_planets_1.map(p => p.name).join(', ') + ' im D60 Lagna — Kernkarma in diesen Planeten konzentriert.';
        ...
        if (sunD60) {
            ...
            if (sunD.deity) {
            ch2 += '<br>' + (isEasy ?
                (sunD.deity.nature === 'benefic' ? 'Du hast deinen wahren Zweck gut verfolgt, Selbstverwirklichung kommt natürlich. Sei selbstbewusst!' : 'Es gab Verwirrung über wer du bist. Dein wahres Ich zu finden ist eine wichtige Reise.') :
                (sunD.deity.nature === 'benefic' ?
                    'Seelenzweck wurde richtig verfolgt, Selbstverwirklichung kommt natuerlich.' :
                    'Herausforderungen an Ego und Autoritaet, das wahre Selbst finden ist die Aufgabe.'));
        }
        ...
        if (moonD60) {
            ...
            if (moonD.deity) {
            ch3 += '<br>' + (isEasy ?
                (moonD.deity.nature === 'benefic' ? 'Dein Geist war friedlich, du bist emotional stabil mit starker Intuition.' : 'Es gibt Spuren emotionaler Schwierigkeiten. Meditation und Wasser in der Nähe hilft bei der Heilung.') :
                (moonD.deity.nature === 'benefic' ?
                    'Geist war friedlich, emotionale Stabilitaet und Intuition sind angeboren.' :
                    'Emotionale Wunden bleiben im Unterbewusstsein. Meditation und Ruhe am Wasser helfen.'));
        }
        ...
        if (d60H2planets.length > 0) {
            ...
            d60H2planets.forEach(p => {
                ch6 += p.symbol + ' ' + p.name + deityTag(getDeity(p.sidereal)) + ' — ' + (p.natural === 'benefic' ? 'Gutes Wohlstandskarma. Ueberfluss.' : 'Wohlstandsherausforderung. Durch Anstrengung ueberwinden.') + '<br>';
            });
        }
        ...
        if (!isEasy) html += subChapter('🕉️', 'Götterliste', ch7);
        ...
        const d2LagnaInterp = ['Selbst erarbeiteter Wohlstand. Unabhaengiges und aggressives Investieren.','Sensorische Investition und stabiler Wohlstand. Immobilien, Gastronomie, Kunst.','Verdienst durch intellektuelle Taetigkeit. Schreiben, Bildung, Geschaeftssinn.','Immobilien und Familieneinkommen. Eigentum von der Mutter. Emotionale Ausgaben beachten.','Wohlstand durch Fuehrung und Autoritaet. Regierung, Gold. Prunkvolle Ausgaben.','Einkommen durch Analyse und Faehigkeiten. Medizin, Buchhaltung. Sparsamer Verwalter.','Wohlstand durch Partnerschaft. Recht, Diplomatie, Mode, Kunst.','Wohlstandsaufbau durch fremdes Geld (Erbschaft, Versicherung). Verborgene Quellen.','Einkommen durch Bildung, Ausland, Religion. Glueck bringt Wohlstand.','Systematische Anstrengung baut Wohlstand auf. Langsam aber sicher. Reich nach der Lebensmitte.','Einkommen durch Technologie, Innovation, Netzwerke. Unkonventionelle Quellen.','Einkommen durch spirituelle/kuenstlerische Aktivitaeten. Auslandsbezogener Wohlstand.'][dLagnaSign];

        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '💰 Wohlstandsanalyse' : '💰 D2 Hora — Wohlstandsanalyse') + '</div><div class="interp-text">';
        ...
        if (sunD2) if (!isEasy) html += '<strong>☉ Sun → ' + SIGNS[sunD2.dSign] + ':</strong> ' + (sunD2.dSign === 4 ? '🌟 <strong>Sonne in eigener Hora (Loewe)!</strong> Selbstgemachter Typ. Baut Wohlstand durch Autoritaet und Fuehrung.' : 'Sonne in Mond-Hora. Einkommen durch Hilfe anderer oder oeffentlichen Sektor.') + '<br>';
        if (moonD2) if (!isEasy) html += '<strong>☽ Moon → ' + SIGNS[moonD2.dSign] + ':</strong> ' + (moonD2.dSign === 3 ? '🌟 <strong>Mond in eigener Hora (Krebs)!</strong> Reichhaltiges Leben durch Menschen und Beziehungen.' : 'Mond in Sonnen-Hora. Lebensunterhalt durch eigene Anstrengung.') + '<br>';
        const d2H2sign = (dLagnaSign + 1) % 12;
        const d2H2planets = dPositions.filter(p => p.dSign === d2H2sign);
        html += '<br><strong>D2 2. Haus (Angesammelter Wohlstand) — ' + SIGNS[d2H2sign] + ':</strong><br>';
        if (d2H2planets.length > 0) {
            const wealth = {Sun:'Wohlstand durch Autoritaet und Status',Moon:'Wohlstand durch oeffentliche Aktivitaeten',Mars:'Eigentum, Technologie, Wettbewerbsfelder',Mercury:'Geschaeft, intellektuelle Aktivitaet, Kommunikation',Jupiter:'Bildung, Recht, Religion — reichlicher Wohlstand',Venus:'Kunst, Mode, Luxusgueter',Saturn:'Langsame aber stetige Ansammlung. Stabil nach der Lebensmitte',Rahu:'Unkonventionelle Methoden, auslandsbezogen',Ketu:'Von Materiellem losgeloest. Verfolgt spirituelle Werte'};
            d2H2planets.forEach(p => { html += isEasy ? (wealth[p.id]||'') + '<br>' : '• ' + p.name + ': ' + (wealth[p.id]||'') + '<br>'; });
        } else html += isEasy ? '' : 'Keine Planeten im 2. — Position des Herrschers ist der Schluessel.<br>';
        html += '</div></div>';

    } else if (division === 3) {
        const d3LagnaInterp = ['Unabhaengig, Fuehrung unter Geschwistern. Mutiger Kommunikationsstil.','Stabile, materiell bequeme Geschwisterbeziehungen. Kuenstlerische Geschwister moeglich.','Intellektuelle, kommunikative Geschwister. Viele Gespraeche.','Emotional tiefe Geschwisterbindung. Schuetzende Geschwister.','Charismatische, stolze Geschwister. Beruehmtes oder erfolgreiches Geschwister.','Analytische, praktische Geschwister. Medizin/Bildungsbereich.','Diplomatische, charmante Geschwister. Soziale Verbindungen.','Intensive, geheimnisvolle Geschwisterbeziehungen. Tiefe Bindungen nach Konflikten.','Freie, philosophische Geschwister. Geschwister im Ausland.','Verantwortungsvolle, ehrgeizige Geschwister. Pflichtgefuehl.','Einzigartige, unabhaengige Geschwister. Unkonventionelle Beziehungen.','Spirituelle, kuenstlerische Geschwister. Geschwister im Ausland.'][dLagnaSign];
        const d3_3sign = (dLagnaSign + 2) % 12;
        const d3_3planets = dPositions.filter(p => p.dSign === d3_3sign);

        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '👫 Geschwister & Mut' : '👫 D3 Drekkana — Geschwister & Mut') + '</div><div class="interp-text">';
        html += '<strong>D3 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d3LagnaInterp + '<br><br>';
        html += '<strong>D3 3. Haus (Juengere Geschwister) — ' + SIGNS[d3_3sign] + ':</strong><br>';
        ...
        } else html += isEasy ? '' : 'Keine Planeten im 3. — Position des Herrschers pruefen.<br>';
        html += '</div></div>';

    } else if (division === 4) {
        const d4LagnaInterp = ['Erwirbt aktiv Eigentum. Baut oder kauft gerne neue Haeuser.','Stabiler, reichlicher Immobilienbesitz. Land und Bauernhoefe. Luxurioese Wohnung.','Mehrere Haeuser oder haeufige Umzuege. Bevorzugt intellektuelles Umfeld.','Heim und Eigentum sind emotional wichtig. Nahe am Wasser. Eigentum von der Mutter.','Grosses, geraeumiges Heim. Luxurioese Einrichtung. Prestigetraechtige Gegend.','Saubere, praktische Wohnung. Mehrere kleine Immobilien.','Schoenes, harmonisches Heim. Interesse an Innendesign.','Eigentum durchlaeuft Transformation. Geerbtes Eigentum. Geheime Orte.','Grosses Land und auslaendisches Eigentum. Nahe Bildungseinrichtungen.','Systematische Immobilieninvestition. Alte Gebaeude. Sicheres Wachstum.','Einzigartiger Wohnstil. Moderne Wohnung. Technologiebezogen.','Schoenes Heim am Wasser. Auslaendisches Eigentum. Spiritueller Raum.'][dLagnaSign];
        const d4_4sign = (dLagnaSign + 3) % 12;
        const d4_4planets = dPositions.filter(p => p.dSign === d4_4sign);

        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '🏠 Eigentum & Glück' : '🏠 D4 Chaturthamsa — Eigentum & Glück') + '</div><div class="interp-text">';
        html += '<strong>D4 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d4LagnaInterp + '<br><br>';
        html += '<strong>D4 4. Haus (Eigentum) — ' + SIGNS[d4_4sign] + ':</strong><br>';
        ...
        } else html += isEasy ? '' : 'Keine Planeten im 4. — Position des Herrschers ist der Schluessel.<br>';
        html += '</div></div>';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '📚 Bildung' : '📚 D24 Chaturvimsamsa — Bildung') + '</div><div class="interp-text">';
        html += '<strong>D24 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d24LagnaInterp + '<br><br>';
        html += '<strong>D24 4. (Grundbildung) — ' + SIGNS[d24_4sign] + ':</strong><br>';
        ...
        } else html += isEasy ? '' : 'Keine Planeten im 4.<br>';
        if (jupD24) { const jH = ((jupD24.dSign - dLagnaSign + 12) % 12) + 1; html += '<br><strong>♃ Jupiter (Weisheit) → ' + jH + 'H:</strong> ' + ([1,4,5,9].includes(jH) ? '🎓 <strong>Hohe akademische Leistung erwartet!</strong> Aufbaustudium/Promotion/Auslandsstudium moeglich.' : 'Wachstum durch Lernen. Jupiter-Segen im Haus ' + jH + '.') + '<br>'; }
        if (merD24) { const mH = ((merD24.dSign - dLagnaSign + 12) % 12) + 1; html += '<strong>☿ Merkur (Lernen) → ' + mH + 'H:</strong> ' + ([1,4,5,9].includes(mH) ? '📖 <strong>Herausragende intellektuelle Faehigkeit!</strong> Talent in Mathematik, Sprachen, Analyse.' : 'Intellektuelle Faehigkeit im Haus ' + mH + '.') + '<br>'; }
        html += '</div></div>';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '⚠️ Unglück & Krankheit' : '⚠️ D30 Trimsamsa — Unglück & Krankheit') + '</div><div class="interp-text">';
        html += '<strong>D30 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d30LagnaInterp + '<br><br>';
        const diseaseBySign = ['Kopf, Gehirn, Fieber, Entzuendung','Hals, Schilddruese, Diabetes','Lunge, Nerven, Angst','Stomach, water retention','Herz, Ruecken, Blutdruck','Digestive, intestines, skin','Kidneys, lower back, urinary','Reproductive, chronic disease','Liver, thighs, overweight','Knochen, Gelenke, Rheuma','Circulation, blood pressure, ankles','Immunsystem, Fuesse, psychische Gesundheit'];
        html += '<strong>D30 6. (Krankheit) — ' + SIGNS[d30_6sign] + ':</strong><br>';
        ...
        html += '<br><strong>D30 8. (Gefahr) — ' + SIGNS[d30_8sign] + ':</strong><br>';
        if (d30_8planets.length > 0) {
            d30_8planets.forEach(p => { html += isEasy ? (p.natural === 'malefic' ? 'Gefahr/Unfallvorsicht. Versicherung und Checkups wichtig.' : 'In Krisen geschuetzt.') + '<br>' : '• ' + p.name + ': ' + (p.natural === 'malefic' ? 'Gefahr/Unfallvorsicht. Versicherung und Checkups wichtig.' : 'In Krisen geschuetzt.') + '<br>'; });
        } else html += isEasy ? '' : 'Keine Planeten im 8. — geringes Risiko.<br>';
        html += '</div></div>';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '👩 Mütterliches Erbe' : '👩 D40 Khavedamsa — Mütterliches Erbe') + '</div><div class="interp-text">';
        html += '<strong>D40 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d40LagnaInterp + '<br>';
        if (moonD40) { const mH = ((moonD40.dSign - dLagnaSign + 12) % 12) + 1; html += '<br><strong>☽ Mond (Mutter-Karaka) → ' + mH + 'H:</strong> ' + ['','Starker muetterlicher Einfluss','Eigentum von der Mutter','Gute Kommunikation mit der Mutter','Tiefe Bindung zur Mutter! Beste Platzierung','Kreative Mutter','Dienstorientierte Mutter','Mutter beeinflusst Beziehungen','Erbschaft von der Mutter','Religioese/paedagogische Mutter','Sozial erfolgreiche Mutter','Unabhaengige Mutter','Spirituelle Mutter'][mH] + '<br>'; }
        html += '</div></div>';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '👨 Väterliches Erbe' : '👨 D45 Akshavedamsa — Väterliches Erbe') + '</div><div class="interp-text">';
        html += '<strong>D45 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d45LagnaInterp + '<br>';
        if (sunD45) { const sH = ((sunD45.dSign - dLagnaSign + 12) % 12) + 1; html += '<br><strong>☉ Sonne (Vater-Karaka) → ' + sH + 'H:</strong> ' + ['','Starker vaeterlicher Einfluss','Eigentum vom Vater','Gute Kommunikation mit dem Vater','Familienorientierter Vater','Kreativer Vater','Dienstorientierter Vater','Vater beeinflusst Beziehungen','Erbschaft vom Vater','Religioeser/paedagogischer Vater','Sozial erfolgreicher Vater! Beste Platzierung','Unabhaengiger Vater','Spiritueller Vater'][sH] + '<br>'; }
        html += '</div></div>';
    }
