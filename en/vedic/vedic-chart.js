...
function renderD9Interpretation(d9Positions, d9LagnaSign, d1LagnaSign) {
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ...
            ${d9LagnaSign === d1LagnaSign ? isEasy ? '<br><br><strong>Special sign!</strong> Your essence remains unchanged after marriage — your inner and outer self are perfectly aligned.' : '<br><br><strong>D1 and D9 Lagna are in the same sign!</strong> This is called <strong>Vargottama</strong> — extremely powerful. Your essence remains unchanged after marriage, inner and outer self are aligned.' : ''}
            ${d9H1Planets.length > 0 ? isEasy ? '<br><br>There are energies that strongly influence your personality after marriage.' : '<br><br><strong>Planets in D9 1st house:</strong> ' + d9H1Planets.map(p => p.symbol + ' ' + p.name).join(', ') + ' — These planets strongly influence your personality after marriage.' : ''}
        </div>
    </div>`;
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ${isEasy ? 'Your spouse\'s core personality:' : 'Navamsa 7th house is in <strong>' + SIGNS[d9H7Sign] + '</strong>, ruled by <strong>' + RULER_NAMES[d9H7Ruler] + '</strong>.<br><br>This reveals your spouse\'s core personality — someone with the energy of ' + SIGNS[d9H7Sign] + '.'}
            ${d9H7Planets.length > 0 ? isEasy ? '<br><br>' + d9H7Planets.map(p => p.natural === 'benefic' ? 'Positive energy! You receive blessings from your spouse.' : 'Challenge energy — but also opportunities for growth in marriage.').join('<br>') : '<br><br><strong>Planets in D9 7th house:</strong><br>' + d9H7Planets.map(p => `${p.symbol} <strong>${p.name}</strong>: ${p.natural === 'benefic' ? 'Benefic! You receive positive energy from your spouse.' : 'Malefic — challenges in marriage, but also opportunities for growth.'}`).join('<br>') : '<br><br>No planets in 7th house — the position of the 7th lord matters more.'}
        </div>
    </div>`;
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ${isEasy ? 'The true calling you pursue after maturity.' : 'Navamsa 10th house is in <strong>' + SIGNS[d9H10Sign] + '</strong>, ruled by <strong>' + RULER_NAMES[d9H10Ruler] + '</strong>.<br><br>While D1\'s 10th shows your career, D9\'s 10th reveals your <strong>greater life purpose (Dharma)</strong> — the true calling you pursue after maturity.'}<br><br>
            <strong>Direction of purpose:</strong> ${careerBySgn[d9H10Sign]}
            ${d9H10Planets.length > 0 ? isEasy ? '<br><br>' + d9H10Planets.map(p => planetCareer[p.id] || 'Unique career energy').join('<br>') : '<br><br><strong>Planets in D9 10th house:</strong><br>' + d9H10Planets.map(p => `${p.symbol} <strong>${p.name}</strong>: ${planetCareer[p.id] || 'Unique career energy'}`).join('<br>') : ''}
        </div>
    </div>`;

    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '👔 Spouse Career' : '👔 Spouse Career — Derived 10th (D9 4th House): ' + SIGNS[d9H4Sign] + ' ' + SIGN_SYMBOLS[d9H4Sign]}</div>
        <div class="interp-text">
            ${isEasy ? '' : '<strong>Derived house principle:</strong> The 10th from 7th (spouse) = D9\'s 4th house shows your spouse\'s career/social activity.<br><br>D9 4th house is in <strong>' + SIGNS[d9H4Sign] + '</strong>, ruled by <strong>' + RULER_NAMES[d9H4Ruler] + '</strong>.<br><br>'}
            <strong>Spouse career tendency:</strong> ${careerBySgn[d9H4Sign]}
            ${d9H4Planets.length > 0 ? isEasy ? '<br><br>' + d9H4Planets.map(p => `Spouse likely works in ${planetCareer[p.id] || 'specialized field'}`).join('<br>') : '<br><br><strong>Planets in D9 4th (spouse 10th):</strong><br>' + d9H4Planets.map(p => `${p.symbol} <strong>${p.name}</strong>: Spouse likely works in ${planetCareer[p.id] || 'specialized field'}`).join('<br>') : ''}
        </div>
    </div>`;
    ...
    if (vargottamaPlanets.length > 0) {
        html += `<div class="interp-card">
            <div class="interp-title">${isEasy ? '⭐ Exceptionally Strong Energies' : '⭐ Vargottama Planets — Exceptionally Strong'}</div>
            <div class="interp-text">
                ${isEasy ? 'These energies are exceptionally powerful and act consistently throughout your life.' : 'Planets in the same sign in both D1 and D9 are called <strong>Vargottama</strong>. These are very powerful, their energy acts consistently throughout life.'}<br><br>
                ${isEasy ? 'Exceptionally strong energy acts consistently throughout your life!' : vargottamaPlanets.map(p => `<strong>${p.symbol} ${p.name}</strong>: In ${SIGNS[p.sign]} in both D1 and D9 — exceptionally strong energy!`).join('<br>')}
            </div>
        </div>`;
    }
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ${isEasy ? 'Analysis of which direction your spouse may come from.<br><br>' : 'Vedic astrology determines spouse direction by combining multiple indicators.<br><br>'}
            ${isEasy ? '' : '<strong>6 Indicators:</strong><br>' + dirSources.map(s => `• <strong>${s.name}</strong>: ${SIGNS[s.sign]} ${SIGN_SYMBOLS[s.sign]} → <strong>${DIRECTIONS[s.sign]}</strong> <span style="color:#666;font-size:12px;">(${s.desc})</span>`).join('<br>')}
            <br><br>
            ${isEasy ? '' : '<strong>🧿 Upapada Lagna (UL):</strong> Arudha of 12th — spouse family/background → <strong>' + SIGNS[ulSign] + ' ' + SIGN_SYMBOLS[ulSign] + '</strong><br><strong>🎯 Darapada (A7):</strong> Arudha of 7th — spouse social image → <strong>' + SIGNS[a7Sign] + ' ' + SIGN_SYMBOLS[a7Sign] + '</strong><br><strong>💍 D9 7th Lord (' + RULER_NAMES[d9H7Ruler] + '):</strong> Where the Navamsa 7th lord sits → <strong>' + SIGNS[d9H7RulerSign] + ' ' + SIGN_SYMBOLS[d9H7RulerSign] + '</strong><br><strong>♀ D9 Venus:</strong> Spouse karaka in Navamsa → <strong>' + SIGNS[venusD9Sign] + ' ' + SIGN_SYMBOLS[venusD9Sign] + '</strong><br><br>'}
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🤝 Where You Meet Your Spouse' : '🤝 Where You Meet Your Spouse — D1 7th: ' + SIGNS[d1H7ForMeeting] + ' ' + SIGN_SYMBOLS[d1H7ForMeeting]}</div>
        <div class="interp-text">
            ${isEasy ? '' : 'The 7th house sign reveals the environment and circumstances of meeting your spouse.<br><br>'}
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🏛️ Spouse Family Background' : '🏛️ Spouse Family Background — UL: ' + SIGNS[ulSign] + ' ' + SIGN_SYMBOLS[ulSign]}</div>
        <div class="interp-text">
            ${isEasy ? '' : 'Upapada Lagna (UL) reveals your spouse\'s family environment and upbringing.<br><br>'}
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">
            ${isEasy ? '' : 'Darapada (A7) shows how your spouse appears to the world — their external image and first impression.<br><br>'}
            <strong>${imageBySgn[a7Sign]}</strong>
        </div>
    </div>`;

    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💎 Spouse Attraction Point' : '💎 Spouse Attraction Point — D9 Venus: ' + SIGNS[venusD9Sign] + ' ' + SIGN_SYMBOLS[venusD9Sign]}</div>
        <div class="interp-text">
            ${isEasy ? '' : 'Venus in Navamsa reveals your spouse\'s core charm and love style.<br><br>'}
...
function renderInterpretation(positions, lagnaSign, moonPos) {
    ...
    if (h2planets.length === 0) {
        wealthText += isEasy ? 'Wealth accumulation is steady and stable. The type that builds up little by little. ' : 'No planets in the 2nd house — wealth accumulation is steady but stable without major fluctuations. ';
    } else {
        h2planets.forEach(p => {
            const pWealth = {
    ...
    if (h11planets.length === 0) {
        wealthText += isEasy ? 'Income is stable but without major fluctuations.' : 'No planets in the 11th house — income is stable but without major fluctuations.';
    } else {
    ...
    let spouseText = (isEasy ? '' : '<strong>📐 Spouse Appearance & First Impression:</strong><br>') + spouseSign[h7sign];
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💍 My Spouse' : '💕 Spouse & Marriage Fortune — 7th House: ' + SIGNS[h7sign] + ' ' + SIGN_SYMBOLS[h7sign]}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💼 My Career' : '💼 Career & Social Achievement — 10th House: ' + SIGNS[h10sign] + ' ' + SIGN_SYMBOLS[h10sign]}</div>
    ...
    html += `<div class="interp-card">
        ...
        <div class="interp-text">${healthByLagna[lagnaSign]}${h6planets.length > 0 ? isEasy ? '<br><br>Special attention to health management is needed.' : '<br><br>' + h6planets.map(p => p.name).join(', ') + ' in the 6th house requires special attention to health management.' : ''}</div>
    </div>`;
    ...
    if (moonPos && jupiter) {
        ...
        if (diff === 0 || diff === 3 || diff === 6 || diff === 9) {
            yogaText += isEasy ? '<strong>📚 Outstanding Intelligence Blessing</strong>' : '<strong>🐘 Gajakesari Yoga</strong> — Moon and Jupiter in Kendra relationship! A combination of wisdom, fame, and abundance. Socially respected with exceptional intellectual abilities. Good education and children fortune.<br><br>';
        }
    }
    ...
    if (sun && mercury && sun.sign === mercury.sign) {
        yogaText += isEasy ? '<strong>📚 Outstanding Intelligence Blessing</strong>' : '<strong>📚 Budha-Aditya Yoga</strong> — Sun and Mercury in the same sign! Outstanding intellect and communication skills. Success in education, writing, and business. An authoritative intellectual leader.<br><br>';
    }
    ...
    if (mars) {
        ...
        if ([1,2,4,7,8,12].includes(marsH)) {
            yogaText += isEasy ? `<strong>⚠️ Marriage Caution</strong>` : `<strong>⚠️ Kuja Dosha (Manglik)</strong> — Mars is positioned in the ${marsH}${marsH===1?'st':marsH===2?'nd':marsH===3?'rd':'th'} house, which may bring challenges in married life. It is advisable to check your partner's chart as well when choosing a spouse. Marriage after age 28 may be more favorable.<br><br>`;
        }
    }
...
function renderDivisionalChart(positions, lagnaSidereal, division, chartId, interpId, label, koName) {
    ...
    if (division === 10) {
        ...
        if (d10_10planets.length > 0 && !isEasy) {
        ...
        html += isEasy ? '' : '<strong>D7 Lagna:</strong> ' + SIGNS[dLagnaSign] + '<br>';
        ...
        html += isEasy ? '' : '<strong>D12 Lagna:</strong> ' + SIGNS[dLagnaSign] + '<br>';
        ...
        if (lagnaD.deity) {
            ch1 += '<br>' + (isEasy ?
                (lagnaD.deity.nature === 'benefic' ? 'You did many good things in past lives, so good opportunities naturally come to you. Your very existence is protected.' : 'There are unresolved lessons from past lives affecting your personality, but overcoming them leads to even greater growth.') :
        ...
        if (sunD60) {
            ...
            let ch2 = (isEasy ? '<strong>Soul Purpose</strong>' : '<strong>D60 Sun: ' + SIGNS[sunD60.dSign] + ' ' + SIGN_SYMBOLS[sunD60.dSign] + '</strong>' + deityTag(sunD) + '<br><br>') + '<br><br>';
            ch2 += (d60SunInterp[sunD60.dSign] || '') + '<br>';
            if (sunD.deity) {
            ch2 += '<br>' + (isEasy ?
                (sunD.deity.nature === 'benefic' ? 'You pursued your true purpose well in past lives, so self-realization comes naturally. Be confident!' : 'There was confusion about who you are in past lives. Finding your true self is an important journey that will make you grow.') :
                (sunD.deity.nature === 'benefic' ?
                    'Soul purpose was rightly pursued, self-realization comes naturally.' :
                    'Challenges to ego and authority in past life, finding true self is the task.'));
        }
        ...
        if (moonD60) {
            ...
            let ch3 = (isEasy ? '<strong>Emotional Memory</strong>' : '<strong>D60 Moon: ' + SIGNS[moonD60.dSign] + ' ' + SIGN_SYMBOLS[moonD60.dSign] + '</strong>' + deityTag(moonD) + '<br><br>') + '<br><br>';
            ch3 += (d60MoonInterp[moonD60.dSign] || '') + '<br>';
            if (moonD.deity) {
            ch3 += '<br>' + (isEasy ?
                (moonD.deity.nature === 'benefic' ? 'Your mind was peaceful in past lives, so you are emotionally stable with strong intuition. Trust your gut feelings.' : 'There are traces of emotional hardship from past lives deep in your heart. Meditation and being near water greatly helps healing.') :
                (moonD.deity.nature === 'benefic' ?
                    'Mind was peaceful in past life, emotional stability and intuition are innate.' :
                    'Emotional wounds remain in subconscious. Meditation and rest near water help.'));
        }
        ...
        let ch4 = isEasy
            ? '<strong>Spouse Karma</strong><br><br>'
            : '<strong>D60 7th House: ' + SIGNS[d60H7sign] + ' ' + SIGN_SYMBOLS[d60H7sign] + '</strong> (7th Lord: ' + (RULER_NAMES[d60H7lord]||d60H7lord) + ')<br><br>';
        ...
        if (venusD60) {
            ...
            if (!isEasy) ch4 += '<br><strong>♀ Venus (Love Karaka)</strong> → D60 ' + venH + 'H (' + houseThemes[venH] + ')' + deityTag(venD) + '<br>';
            ch4 += isEasy ?
                (venD.deity && venD.deity.nature === 'benefic' ? 'You loved sincerely in past lives, so beautiful love awaits you.' : 'There are unresolved love lessons from past lives. Learning what true love means is important.') :
                (venD.deity && venD.deity.nature === 'benefic' ? 'Venus under benefic protection. Love was rightly practiced, beautiful love awaits.' : 'Venus under malefic influence. Learning the true meaning of love is the task.');
        }
        if (rahuD60 && ketuD60) {
            ...
            if (rahuH === 7 || ketuH === 7 || rahuH === 1 || ketuH === 1) {
                ch4 += isEasy ? '<br><br>🔥 <strong>Rahu-Ketu axis on 1-7 line!</strong>' : '<br><br>🔥 <strong>Rahu-Ketu axis on 1-7 line!</strong> Very strong past life connection with spouse. Destined to meet.';
            }
        }
        ...
        let ch5 = isEasy
            ? '<strong>Career Karma</strong><br><br>'
            : '<strong>D60 10th House: ' + SIGNS[d60H10sign] + ' ' + SIGN_SYMBOLS[d60H10sign] + '</strong> (10th Lord: ' + (RULER_NAMES[d60H10lord]||d60H10lord) + ')<br><br>';
        ...
        if (satD60) {
            ...
            if (!isEasy) ch5 += '<br><strong>♄ Saturn (Lord of Karma)</strong> → D60 ' + satH + 'H (' + houseThemes[satH] + ')' + deityTag(satD) + '<br>';
            ch5 += isEasy ?
        ...
        let ch6 = isEasy
            ? '<strong>Wealth Karma</strong><br><br>'
            : '<strong>D60 2nd House: ' + SIGNS[d60H2sign] + ' ' + SIGN_SYMBOLS[d60H2sign] + '</strong><br><br>' + wealthKarma + '<br>';
        ...
        let ch8 = isEasy ?
            '9 planets: <strong style="color:#5cb85c">' + beneficCount + ' good energy</strong>, <strong style="color:#d9534f">' + (positions.length - beneficCount) + ' caution energy</strong><br><br>' :
            'Out of 9 planets: <strong style="color:#5cb85c">' + beneficCount + ' benefic</strong>, <strong style="color:#d9534f">' + (positions.length - beneficCount) + ' malefic</strong><br><br>';
        if (beneficCount >= 7) ch8 += isEasy ?
                '🌟 <strong>You did so many good things in past lives!</strong> Almost all energies are positive, so you naturally get good results in this life.' :
                '🌟 <strong>Very strong past life merit.</strong> Parashara called this "a soul blessed by the gods." Most planets under benefic deities.';
        else if (beneficCount >= 5) { ch8 += isEasy ?
                '✨ <strong>Abundant good energy from past lives.</strong> You\'re protected in many areas of life.' :
                '✨ <strong>Abundant past life merit.</strong> Benefic predominate, protection in many areas.'; if (maleficPlanets.length > 0) ch8 += ' Watch: <strong>' + maleficPlanets.map(p => p.name).join(', ') + '</strong> — practice mantras and charity.'; }
        else if (beneficCount >= 3) { ch8 += isEasy ?
                '⚖️ <strong>Good and challenging energy are about half-half.</strong> Good things and challenges alternate in life.' :
                '⚖️ <strong>Balanced karma.</strong> Good events and challenges alternate.'; if (maleficPlanets.length > 0) ch8 += '<br>Watch: <strong>' + maleficPlanets.map(p => p.name).join(', ') + '</strong>'; }
        ...
        if (sunD2) if (!isEasy) html += '<strong>☉ Sun → ' + SIGNS[sunD2.dSign] + ':</strong> ' + (sunD2.dSign === 4 ? '🌟 <strong>Sun in own hora (Leo)!</strong> Self-made type. Builds wealth through authority and leadership.' : 'Sun in Moon hora. Income through others help or government/public sector.') + '<br>';
        if (moonD2) if (!isEasy) html += '<strong>☽ Moon → ' + SIGNS[moonD2.dSign] + ':</strong> ' + (moonD2.dSign === 3 ? '🌟 <strong>Moon in own hora (Cancer)!</strong> Abundant life through people and relationships.' : 'Moon in Sun hora. Livelihood through own effort and independent activity.') + '<br>';
        const d2H2sign = (dLagnaSign + 1) % 12;
        const d2H2planets = dPositions.filter(p => p.dSign === d2H2sign);
        html += isEasy ? '<br><strong>Accumulated Wealth:</strong><br>' : '<br><strong>D2 2nd House (Accumulated Wealth) — ' + SIGNS[d2H2sign] + ':</strong><br>';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '👫 Siblings & Courage' : '👫 D3 Drekkana — Siblings & Courage') + '</div><div class="interp-text">';
        html += '<strong>D3 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d3LagnaInterp + '<br><br>';
        html += isEasy ? '<strong>Younger Siblings:</strong><br>' : '<strong>D3 3rd House (Younger Siblings) — ' + SIGNS[d3_3sign] + ':</strong><br>';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '🏠 Property & Fortune' : '🏠 D4 Chaturthamsa — Property & Fortune') + '</div><div class="interp-text">';
        html += '<strong>D4 Lagna: ' + SIGNS[dLagnaSign] + '</strong><br>' + d4LagnaInterp + '<br><br>';
        html += isEasy ? '<strong>Property:</strong><br>' : '<strong>D4 4th House (Property) — ' + SIGNS[d4_4sign] + ':</strong><br>';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '📚 Education Analysis' : '📚 D24 Chaturvimsamsa — Education') + '</div><div class="interp-text">';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '⚠️ Health Caution Details' : '⚠️ D30 Trimsamsa — Misfortune & Disease') + '</div><div class="interp-text">';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '👩 Maternal Heritage' : '👩 D40 Khavedamsa — Maternal Legacy') + '</div><div class="interp-text">';
        ...
        html += '<div class="interp-card"><div class="interp-title">' + (isEasy ? '👨 Paternal Heritage' : '👨 D45 Akshavedamsa — Paternal Legacy') + '</div><div class="interp-text">';
