...
function updateCatHeaders() {
    ...
    var ids = {
        catGuide: e ? 'Glossário' : 'Guia de Astrologia Védica',
        catBasic: e ? 'Minhas Posições Planetárias' : 'Carta Básica — Posições & Carta Natal',
        catDasha: e ? 'Meus Períodos de Vida' : 'Dasha — Períodos de Vida',
        catInterp: e ? 'Minha Leitura — Personalidade·Riqueza·Carreira·Saúde' : 'Interpretação — Personalidade·Riqueza·Carreira·Saúde·Yoga',
        catMarriage: e ? 'Meu Cônjuge' : 'Casamento & Cônjuge — D9 Navamsha',
        ...
        catWarn: e ? 'Precauções de Saúde' : 'Precauções — D30 Doença·Estrangeiro',
        catKarma: e ? 'Karma de Vidas Passadas' : 'Karma — D60 Vidas Passadas',
    };
...
function renderNakshatra(moonPos) {
    ...
    const html = isEasy ? `
        <div class="nakshatra-card">
            <div class="nakshatra-name">Sua Estrela: ${nak.ko || nak.name}</div>
...
function renderDasha(moonNakshatra, birthDate, moonSidereal) {
    ...
    html += isEasy ? '</div></div>' : '🌙 Lua natal: <strong>' + nak.ko + ' (' + nak.name + ')</strong> — Primeiro Dasha: <strong>' + DASHA_KO[startRuler] + '</strong> (restante: ' + remainingYears.toFixed(2) + ' anos)</div></div>';
...
function renderInterpretation(positions, lagnaSign, moonPos) {
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '👤 Personality & Appearance' : '👤 Personality & Appearance — Lagna: ${SIGNS[lagnaSign]} ${SIGN_SYMBOLS[lagnaSign]}'}</div>
    ...
    if (moonPos) {
        ...
        html += `<div class="interp-card">
            <div class="interp-title">${isEasy ? '🌙 Seu Estilo Emocional' : '🌙 Inner Self & Emotions — Moon: ${SIGNS[moonPos.sign]} ${SIGN_SYMBOLS[moonPos.sign]}'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💰 Minha Fortuna' : '💰 Wealth Fortune'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💍 Meu Cônjuge' : '💕 Spouse & Marriage Fortune — 7th House: ${SIGNS[h7sign]} ${SIGN_SYMBOLS[h7sign]}'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '💼 Minha Carreira' : '💼 Career & Social Achievement — 10th House: ${SIGNS[h10sign]} ${SIGN_SYMBOLS[h10sign]}'}</div>
    ...
    html += `<div class="interp-card">
        <div class="interp-title">${isEasy ? '🏥 Minha Saúde' : '🏥 Health — Vulnerable Areas'}</div>
...
function renderForeign(positions, lagnaSign) {
    ...
    if (h9.length === 0) {
        text += isEasy ? '' : 'Sem planetas na casa 9 — viagens ao exterior existem mas sem conexao particularmente forte.';
    } else {
        h9.forEach(p => {
    ...
    if (h12.length === 0) {
        text += isEasy ? '' : 'Sem planetas na casa 12 — a residencia domestica e mais natural que o assentamento no exterior.';
    } else {
        h12.forEach(p => {
...
function renderDignity(positions, lagnaSign) {
    ...
    positions.forEach(p => {
        ...
        if (p.sign === EXALT[p.id]) {
            ...
            emoji = '🟢';
            color = '#5cb85c';
            simpleDesc = isEasy ? `<strong>${area}</strong> Você recebeu a maior bênção nesta área! Seus talentos inatos brilham.` : `<strong>${p.name} is at maximum power!</strong> The "${role}" energy is maximized in the <strong>${house}${hSuffix} house (${area})</strong> area, bringing great blessings. Innate talents shine and good results come naturally.`;
        } else if (p.sign === DEBI[p.id]) {
            dignity = 'Debilitated';
            emoji = '🔴';
            color = '#d9534f';
            simpleDesc = isEasy ? `<strong>${area}</strong> Você pode enfrentar desafios nesta área. Mas o esforço consciente pode transformar isso em uma grande oportunidade de crescimento.` : `<strong>${p.name} is in a weakened state.</strong> The "${role}" energy is weakened in the <strong>${house}${hSuffix} house (${area})</strong> area. You may experience difficulties in this field, but conscious effort to overcome them can become a great opportunity for growth. See the remedies below.`;
        } else if (OWN[p.id] && OWN[p.id].includes(p.sign)) {
            dignity = 'Own Sign';
            emoji = '🟡';
            color = '#c9a84c';
            simpleDesc = isEasy ? `<strong>${area}</strong> Esta área funciona estavelmente a seu favor. Bons resultados vêm naturalmente.` : `<strong>${p.name} is at home!</strong> The "${role}" energy stably exerts its power in the <strong>${house}${hSuffix} house (${area})</strong> area. Good results come naturally.`;
        } else {
            dignity = 'Neutral';
            emoji = '⚪';
            color = '#999';
            simpleDesc = isEasy ? `<strong>${area}</strong> Influência média nesta área. Nem particularmente forte nem fraco.` : `${p.name}'s "${role}" energy exerts average influence in the <strong>${house}${hSuffix} house (${area})</strong> area. Results vary depending on relationships with other planets.`;
        }
...
function renderDivisionalChart(positions, lagnaSidereal, division, chartId, interpId, label, koName) {
    ...
    if (division === 10) {
        ...
        if (d7_5planets.length === 0) html += isEasy ? '' : 'Sem planetas na casa 5 — verifique a posicao do senhor da casa 5.';
        html += '</div></div>';
        ...
        if (sunD60) {
            ...
            let ch2 = (isEasy ? '<strong>Propósito da Alma</strong>' : '<strong>D60 Sol: ' + SIGNS[sunD60.dSign] + ' ' + SIGN_SYMBOLS[sunD60.dSign] + '</strong>' + deityTag(sunD) + '<br><br>') + '<br><br>';
        ...
        if (moonD60) {
            ...
            let ch3 = (isEasy ? '<strong>Memória Emocional</strong>' : '<strong>D60 Lua: ' + SIGNS[moonD60.dSign] + ' ' + SIGN_SYMBOLS[moonD60.dSign] + '</strong>' + deityTag(moonD) + '<br><br>') + '<br><br>';
        ...
        let ch4 = isEasy
            ? '<strong>Karma do Cônjuge</strong><br><br>'
            : '<strong>D60 Casa 7: ' + SIGNS[d60H7sign] + ' ' + SIGN_SYMBOLS[d60H7sign] + '</strong> (Senhor da casa 7: ' + (RULER_NAMES[d60H7lord]||d60H7lord) + ')<br><br>';
        ...
        if (venusD60) {
            ...
            if (!isEasy) ch4 += '<br><strong>♀ Vênus (Karaka do Amor)</strong> → D60 ' + venH + 'H (' + houseThemes[venH] + ')' + deityTag(venD) + '<br>';
            ch4 += isEasy ?
        ...
        if (rahuD60 && ketuD60) {
            ...
            if (rahuH === 7 || ketuH === 7 || rahuH === 1 || ketuH === 1) {
                ch4 += isEasy ? '<br><br>🔥 <strong>Eixo Rahu-Ketu na linha 1-7!</strong>' : '<br><br>🔥 <strong>Eixo Rahu-Ketu na linha 1-7!</strong> Conexao muito forte de vidas passadas com o conjuge. Destinados a se encontrar.';
            }
        }
        ...
        let ch5 = isEasy
            ? '<strong>Karma Profissional</strong><br><br>'
            : '<strong>D60 Casa 10: ' + SIGNS[d60H10sign] + ' ' + SIGN_SYMBOLS[d60H10sign] + '</strong> (Senhor da casa 10: ' + (RULER_NAMES[d60H10lord]||d60H10lord) + ')<br><br>';
        ...
        if (satD60) {
            ...
            if (!isEasy) ch5 += '<br><strong>♄ Saturno (Senhor do Karma)</strong> → D60 ' + satH + 'H (' + houseThemes[satH] + ')' + deityTag(satD) + '<br>';
            ch5 += isEasy ?
        ...
        let ch6 = isEasy
            ? '<strong>Karma de Riqueza</strong><br><br>'
            : '<strong>D60 Casa 2: ' + SIGNS[d60H2sign] + ' ' + SIGN_SYMBOLS[d60H2sign] + '</strong><br><br>' + wealthKarma + '<br>';
        ...
        let ch8 = isEasy ?
            '9 planets: <strong style="color:#5cb85c">' + beneficCount + ' energia boa</strong>, <strong style="color:#d9534f">' + (positions.length - beneficCount) + ' energia de atenção</strong><br><br>' :
            'De 9 planetas: <strong style="color:#5cb85c">' + beneficCount + ' benefico</strong>, <strong style="color:#d9534f">' + (positions.length - beneficCount) + ' malefico</strong><br><br>';
        if (beneficCount >= 7) ch8 += isEasy ?
                '🌟 <strong>Você fez tantas coisas boas!</strong> Quase toda energia é positiva.' :
                '🌟 <strong>Merito muito forte de vidas passadas.</strong> Parashara chamou isso de "uma alma abencoacada pelos deuses." Maioria dos planetas sob divindades beneficas.';
        else if (beneficCount >= 5) { ch8 += isEasy ?
                '✨ <strong>Energia boa abundante.</strong> Você é protegido/a em muitas áreas.' :
                '✨ <strong>Merito abundante de vidas passadas.</strong> Benefic predominate, protection in many areas.'; if (maleficPlanets.length > 0) ch8 += ' Atencao: <strong>' + maleficPlanets.map(p => p.name).join(', ') + '</strong> — practice mantras and charity.'; }
        else if (beneficCount >= 3) { ch8 += isEasy ?
                '⚖️ <strong>Energia boa e de desafio estão meio a meio.</strong>' :
                '⚖️ <strong>Karma equilibrado.</strong> Good events and challenges alternate.'; if (maleficPlanets.length > 0) ch8 += '<br>Watch: <strong>' + maleficPlanets.map(p => p.name).join(', ') + '</strong>'; }
        ...
        } else html += isEasy ? '' : 'Sem planetas na casa 2 — posicao do senhor e a chave.<br>';
        html += '</div></div>';
